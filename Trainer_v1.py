...
class AutoModel(pl.LightningModule):
    ...
    def training_step(self, batch_data, batch_idx):
        ...
        self.log("train/loss", loss,
                 on_step=True, on_epoch=True, prog_bar=True, logger=True, batch_size=self.train_bsz, sync_dist=True)
        return loss
    ...
    def validation_step(self, batch_data, batch_idx):
        ...
        self.log("val/loss", val_loss, batch_size=self.val_bsz, on_epoch=True, sync_dist=True)
        self.log("val/mIoU", miou, batch_size=self.val_bsz, on_epoch=True, sync_dist=True)
        iou_dict = {}
        for i, iou in enumerate(iou_list):
            word = label2word(i, self.builder.kitti_yaml["labels"], self.builder.kitti_yaml["learning_map_inv"])
            self.log(f"val/iou_{word}", iou, batch_size=self.val_bsz, on_epoch=True, sync_dist=True)
...
if __name__ == "__main__":
    ...
    wandb_logger = WandbLogger(save_dir=experiment_dir, project="cy3d")
